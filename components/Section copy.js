...
export default function Section({children,backgroundImage,backgroundColor,showUpTo,columns}){
    ...
    const grid = column > 1 ? <Grid.Container id={sectionID} className={`${css.section} observed`} gap={1} css={{padding:"5vw",...filling}} justify="space-evenly"> {children.map(child=> <Grid {...deviceGrid[showUpTo]}>{child}</Grid>)} </Grid.Container> : <div id={sectionID} className={`${css.section} observed`} style={filling}>{children}</div>   
